  ...
  static async saveImportedClusterResources(clusterTag, eksClusterName, awsRegion, hyperPodClusters = null, depsStackInfo = null, userSecurityGroup = null) {
    try {
      ...
      if (vpcCidr && !hyperPodCluster) {
        // 导入的 EKS 只需要 hyperPodPrivateSubnetCidr，其他 subnet 已存在
        const [o1, o2] = vpcCidr.split('.').slice(0, 2);
        const cidrConfig = {
          vpcCidr,
          hyperPodPrivateSubnetCidr: `${o1}.${o2}.32.0/20`,
          generatedAt: new Date().toISOString(),
          region: awsRegion,
          autoGenerated: true,
          comment: 'Generated CIDR for compute subnet creation during HyperPod creation (if no existing subnet provided)'
        };
        ...
        console.log(`Saved CIDR configuration for imported cluster: ${clusterTag}, hyperPodPrivateSubnetCidr: ${cidrConfig.hyperPodPrivateSubnetCidr}`);
      }
